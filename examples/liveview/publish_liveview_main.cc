...
int main(int argc, char **argv) {
    ...
    // create liveview publisher
    auto camera = std::string(type_to_str[type]);

    auto liveview_publisher = std::make_shared<LiveviewSample>(std::string(camera));

    StreamDecoder::Options decoder_option = {.name = std::string("ffmpeg")};
    auto stream_decoder = CreateStreamDecoder(decoder_option);

    ImageProcessor::Options image_processor_option = {.name = std::string("publisher"),
                                       .alias = camera, .userdata = liveview_publisher};
    auto image_processor = CreateImageProcessor(image_processor_option);

    if (0 != InitLiveviewSample(
        liveview_publisher, (Liveview::CameraType)type, (Liveview::StreamQuality)quality,
        stream_decoder, image_processor)) {
        ERROR("Init %s liveview publisher failed", camera.c_str());
    } else {
        liveview_publisher->Start();
    }
